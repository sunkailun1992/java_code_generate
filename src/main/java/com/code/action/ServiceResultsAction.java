...
/**
 * ServiceResults 代码生成 Action
 *
 * 生成：
 *  - ServiceResults
 *
 * 兼容 IntelliJ IDEA 2024+
 *
 * @author 孙凯伦
 */
public class ServiceResultsAction extends BaseAnAction {
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        params.put("serviceResults", "y");          // ✅ 生成 ServiceResults
        params.put("serviceQuery", "n");            // 不生成 ServiceQuery
        params.put("mapper", "n");                  // 不生成 Mapper

        // 后台执行生成任务
        ProgressManager.getInstance().run(
                new Task.Backgroundable(project, "Generating ServiceResults Code...", false) {
